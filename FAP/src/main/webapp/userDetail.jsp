            ...
            <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
                <%@page import="models.UserCampus" %>
                    <%@page contentType="text/html" pageEncoding="UTF-8" %>
                        ...
                        <% UserCampus user=(UserCampus) request.getAttribute("user"); User u=(User)
                            session.getAttribute("user"); %>

                            <body>
                                <form class="row g-3 needs-validation" novalidate method="post" action="UserController">
                                    <div class="mb-3">
                                        <input type="hidden" class="form-control" name="check" id="check"
                                            value="delete" />
                                    </div>
                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="userID" id="userID"
                                                    value="<%= user.getUserID().getUserID()%>" readonly>
                                            </div>
                                            <div class="row">
                                                <div class="form-group col-md-6">
                                                    <label for="fistName">First Name</label>
                                                    <input type="text" class="form-control" name="fistName"
                                                        id="fistName" value="<%= user.getUserID().getFirstName()%>"
                                                        readonly>
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label for="lastName">Last Name</label>
                                                    <input type="text" class="form-control" name="lastName"
                                                        id="lastName" value="<%= user.getUserID().getLastName()%>"
                                                        readonly>
                                                </div>
                                            </div>
                                            <div class="row">
                                                ...
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <input type="text" class="form-control" name="phone" id="phone"
                                                        value="<%= user.getUserID().getPhone()%>" readonly>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="email">Email</label>
                                                <input type="text" class="form-control" name="email" id="email"
                                                    value="<%= user.getUserID().getEmail()%>" readonly>
                                            </div>
                                                    ...
                                                    <c:choose>
                                                        <c:when test="<%= user.getUserID().getRole() == 0%>">
                                                            <input type="text" class="form-control" name="role"
                                                                id="role" value="Student" readonly>
                                                        </c:when>
                                                        <c:when test="<%= user.getUserID().getRole() == 1%>">
                                                            <input type="text" class="form-control" name="role"
                                                                id="role" value="Lecturer" readonly>
                                                        </c:when>
                                                        <c:when test="<%= user.getUserID().getRole() == 2%>">
                                                            <input type="text" class="form-control" name="role"
                                                                id="role" value="Academic Staff" readonly>
                                                        </c:when>
                                                        <c:when test="<%= user.getUserID().getRole() == 3%>">
                                                            <input type="text" class="form-control" name="role"
                                                                id="role" value="Administrator" readonly>
                                                        </c:when>
                                                    </c:choose>
                                                </div>
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <input type="text" class="form-control" name="campus" id="campus"
                                                        value="<%= user.getCampusID().getCampusName()%>" readonly>
                                                </div>
                                            </div>
                                            <c:if test="${requestScope.user.userID.role==0}">
                                                <div class="form-group row">
                                                    ...
                                                    <input type="text" class="form-control" name="semester"
                                                        id="semester" value="<%= user.getUserID().getSemester()%>"
                                                        readonly>
                                                </div>
                                                <div class="form-group row">
                                                    ...
                                                    <textarea class="form-control" name="curriculum" id="curriculum"
                                                        rows="3"
                                                        readonly><%= user.getUserID().getCurriculumID().getCurriculumName()%></textarea>
                                                </div>
                                                <div class="form-group row">
                                        ...
                                        <div class="container">
                                            <div class="row">
                                                <c:if test="<%= user.getUserID().getRole() == 0%>">
                                                    <a class="btn btn-primary col-md-3"
                                                        href="UserController?id=<%= user.getUserID().getUserID()%>&check=edit"
                                                        role="button">Edit</a>
                                                    <a class="btn btn-primary col-md-3"
                                                        href="UserController?id=<%= user.getUserID().getUserID()%>&check=login"
                                                        role="button">Login</a>
                                                    <a class="btn btn-primary col-md-3"
                                                        href="UserController?id=<%= user.getUserID().getUserID()%>&check=delete"
                                                        role="button">Delete</a>
                                                    <a class="btn btn-primary col-md-3" href="UserController"
                                                        role="button">Back
                                                        to
                                                        List</a>
                                                </c:if>
                                                <c:if test="<%=user.getUserID().getRole() != 0%>">
                                                    ...
                                                    <a class="btn btn-primary col-md-4"
                                                        href="UserController?id=<%= user.getUserID().getUserID()%>&check=delete"
                                                        role="button">Delete</a>
                                                    <a class="btn btn-primary col-md-4" href="UserController"
                                                        role="button">Back
                                                        to
                                                        List</a>
                                                </c:if>
